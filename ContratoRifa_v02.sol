...
contract ContratoRifa{
    ...
    function selecionaNumerosRifa(uint _idRifa, address payable _participanteComprador, uint _quantNumerosParticipante) private returns (uint _quantNumerosComprados){
        // require (); // Condição se existe algum número disponível

        uint quantNumerosComprados = 0;

        for (uint i = 1; i <= _quantNumerosParticipante; i++){
            if(Rifas[_idRifa].quantNumerosDisponiveis > 0){ // tem número livre na rifa
                quantNumerosComprados = quantNumerosComprados + 1;
                Rifas[_idRifa].numeroParticipante.push(_participanteComprador);
    ...
    function visualizaParticipanteGanhador(uint _idRifa) public view returns(address _participanteGanhador){
        ...
        require(!Rifas[_idRifa].ativaRifa, "A Rifa continua aberta. Concorra.");
        // require se o ganhador já foi sorteado
        require((verificaEndereco(Rifas[_idRifa].participanteGanhador)), "O Ganhador ainda nao foi divugado.");

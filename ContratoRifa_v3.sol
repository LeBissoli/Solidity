...
contract ContratoRifa{
    ...
    struct Rifa { // Funciona como um objeto
        ...
        uint quantNumerosDisponiveis; // total de números disponíveis para venda
    ...
    function criarRifa(string memory _nomeRifa, address payable _enderecoCarteiraRifa, uint _quantNumeros, uint _valorNumero, uint _porcentagemRifa) public payable somenteAdmin { //<- Modificador aqui
    ...
    function gerarGanhadorRifa(uint _idRifa) public somenteAdmin{
        require(Rifas[_idRifa].ativaRifa, "RIFA JA FOI ENCERRADA"); // Exigencia = RIFA está ativa?     
        require(Rifas[_idRifa].participanteGanhador != Rifas[_idRifa].numeroParticipante[0], "A APURACAO JAH ACONTENCEU.");
        ...
        uint valorPremio = ((Rifas[_idRifa].numeroParticipante.length * Rifas[_idRifa].valorNumero) * ( 1 - Rifas[_idRifa].porcentagemRifa)); // O valor da Premiacao
        ...
        fazerPagamentos( _idRifa);

    
    }
    
    function visualizarParticipanteGanhador(uint _idRifa) public view returns(address _participanteGanhador){
    ...
    function fazerPagamentos(uint _idRifa) public somenteAdmin {
